...
const AgendaCard = ({ title, datetime }: AgendaCardProps) => {
  return (
    <div className="col-span-12 xs:col-span-6 sm:col-span-4 flex flex-row items-center gap-x-4 p-4 rounded-md shadow-md">
      <div className="flex-shrink-0 text-center px-6 py-3 rounded-xl bg-[#E6D5C9]">
        <div className="text-[#B09C8E] text-sm">
          {DAYS_OF_THE_WEEK[dayjs(datetime).day()]}
        </div>
        <div className="text-brown-500 text-2xl">{dayjs(datetime).date()}</div>
